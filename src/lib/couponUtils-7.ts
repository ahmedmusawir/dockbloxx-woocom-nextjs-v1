...
export function isCouponExpiredByTimezone(
  ...
  try {
    ...
    // Build comparable strings: "YYYY-MM-DD HH:mm:ss"
    const nowString = `${nowYear}-${nowMonth}-${nowDay} ${nowHour}:${nowMinute}:${nowSecond}`;
    const expiryString = `${expiryDate} ${expiryTime}:00`;

    // Direct string comparison (works because format is sortable)
    const isExpired = nowString > expiryString;
